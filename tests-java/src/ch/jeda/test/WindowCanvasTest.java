...
public class WindowCanvasTest extends Program implements KeyDownListener,
                                                         PointerDownListener {
...
//                jedaColorTest();
    ...
    private void shadowTest() {
        ...
        window.fillRectangle(x, y, w, h);
        drawShadowRectangle(x, y, w, h, 3);
    }

    void drawShadowRectangle(float x, float y, float width, float height, int thickness) {
        int topOpacity = 80;
        int shade = 0;
        for (int i = 0; i < thickness; i++) {
            window.setColor(new Color(shade, shade, shade, topOpacity - ((topOpacity / thickness) * i)));
            window.drawRectangle(x - i, y - i, width + 2 * i, height + 2 * i, Alignment.TOP_LEFT);
        }
    }
